...
/**
 * IndividualModal - Form for creating/editing individual records
 */
const IndividualModal = ({ open, onClose, individual, onSave }) => {
  ...
  // Initialize form data when modal opens or individual changes
  useEffect(() => {
    if (individual) {
      setFormData({
  ...
  const isEditMode = Boolean(individual && individual.id);
  ...
  return (
    ...
    >
      <DialogTitle>
        {isEditMode ? 'Edit Individual' : 'Add New Individual'}
      </DialogTitle>

      <DialogContent dividers>
        <Box sx={{ pt: 1 }}>
          <Grid container spacing={2}>
            {/* Individual ID - shown only when editing */}
            {isEditMode && (
